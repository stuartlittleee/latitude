...
function initScrollAnimations() {
  const animatedElements = document.querySelectorAll(
    '.fade-up, .fade-in, .slide-left, .slide-right, .scale-in, .heading-reveal, .blur-up, .clip-reveal, .slide-up-item'
  );
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // If inside a stagger-children parent, respect its CSS delay (already set)
        // If inside a data-stagger-grid parent, delay is set by JS — skip here
        if (entry.target.closest('[data-stagger-grid]')) return;
...
function initHeroWords() {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      words.forEach(part => {
        if (part.trim() === '') {
          ...
          span.style.animationDelay = (0.08 + counter.val * 0.10) + 's';
          counter.val++;
      ...
      if (node.tagName === 'SPAN') {
        ...
        clone.style.animationDelay = (0.08 + counter.val * 0.10) + 's';
        counter.val++;
      }
      return clone;
    }
  ...
  // Stagger the subtitle, badge, and buttons AFTER the headline words finish
  const totalDelay = 0.08 + counter.val * 0.10 + 0.3;
  const heroSub = document.querySelector('.hero-content-layer p');
  const heroBtns = document.querySelector('.hero-content-layer .flex.flex-col');
  const heroBadge = document.querySelector('.hero-content-layer .section-label');

  [heroBadge, heroSub, heroBtns].forEach((el, i) => {
    if (!el) return;
    el.style.opacity = '0';
    el.style.transform = 'translateY(22px)';
    el.style.transition = 'opacity 0.8s cubic-bezier(0.16,1,0.3,1), transform 0.8s cubic-bezier(0.16,1,0.3,1)';
    el.style.transitionDelay = (totalDelay + i * 0.12) + 's';
    requestAnimationFrame(() => {
      el.style.opacity = '1';
...
function initFeaturedTestimonial() {
  const quoteEl = document.getElementById('testimonial-cinematic-quote');
  const authorEl = document.getElementById('testimonial-cinematic-author');
  const dotsContainer = document.getElementById('testimonial-cinematic-dots');
  ...
  testimonials.forEach((_, i) => {
    ...
    dot.className = 'testimonial-cinematic-dot' + (i === 0 ? ' active' : '');
  ...
  function updateDots() {
    dotsContainer.querySelectorAll('.testimonial-cinematic-dot').forEach((d, i) => {
  ...
  function show(index) {
    ...
    const avatarEl = authorEl.querySelector('.testimonial-cinematic-avatar');
    const nameEl = authorEl.querySelector('.testimonial-cinematic-name');
    const detailEl = authorEl.querySelector('.testimonial-cinematic-detail');
  ...
  // Pause on hover
  const section = quoteEl.closest('.testimonial-cinematic');
  if (section) {
...
/* ---------- Split-Char Color Fill (scroll-driven) ---------- */
function initSplitCharHeadings() {
  // Target section headings with class split-char-target
  // Also auto-apply to h2 inside fade-up sections on home page
  const headings = document.querySelectorAll('.split-char-target, .feature-strip h2, .feature-strip .section-heading-serif');
  const darkHeadings = document.querySelectorAll('.split-char-dark-target');

  function splitHeading(el, dark) {
    if (el.dataset.splitDone) return;
    el.dataset.splitDone = '1';
    const charClass = dark ? 'split-char-dark' : 'split-char';

    // Walk text nodes, wrapping each character
    function wrapChars(node) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        const clone = node.cloneNode(false);
        Array.from(node.childNodes).forEach(child => clone.appendChild(wrapChars(child)));
        return clone;
      }
      return node.cloneNode(true);
    }
    ...
    const frag = document.createDocumentFragment();
    Array.from(el.childNodes).forEach(child => frag.appendChild(wrapChars(child)));
    el.innerHTML = '';
    el.appendChild(frag);
  }

  function lightChars(el, dark) {
    const charClass = dark ? 'split-char-dark' : 'split-char';
    const chars = el.querySelectorAll('.' + charClass);
    chars.forEach((ch, i) => {
      setTimeout(() => ch.classList.add('lit'), i * 28);
    });
  }

  function observeHeadings(list, dark) {
    if (!list.length) return;
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const el = entry.target;
          splitHeading(el, dark);
          // Small delay so split DOM settles
          requestAnimationFrame(() => lightChars(el, dark));
          obs.unobserve(el);
        }
      });
  ...
  observeHeadings(headings, false);
  observeHeadings(darkHeadings, true);
}

/* ---------- Stagger Entrance for Grids ---------- */
function initStaggerGrids() {
  // Any element with data-stagger-grid — each child gets a staggered fade-up
  ...
  grids.forEach(g => {
    // Make sure children have the fade-up class
    Array.from(g.children).forEach(child => {
      if (!child.classList.contains('fade-up') && !child.classList.contains('slide-up-item')) {
        child.classList.add('slide-up-item');
      }
    });
    obs.observe(g);
  });
...
/* ---------- Stat Bar Count Up (home page) ---------- */
function initStatCountUp() {
  const statNums = document.querySelectorAll('.hero-stat-bar .stat-num');
  if (!statNums.length) return;

  // Map display text → numeric target + suffix
  const specs = [
    { target: 1000, suffix: '+', prefix: '' },
    { target: 50,   suffix: '+', prefix: '' },
    { target: 4.9,  suffix: '\u2605', prefix: '', float: true }
  ];
    ...
    entries.forEach(entry => {
      ...
      statNums.forEach((el, i) => {
        ...
        function update(now) {
          ...
          const val = spec.float
            ? (eased * spec.target).toFixed(1)
            : Math.round(eased * spec.target).toLocaleString();
          el.textContent = spec.prefix + val + spec.suffix;
          if (progress < 1) requestAnimationFrame(update);
        }
        requestAnimationFrame(update);
      });
      obs.unobserve(statBar);
    });
  }, { threshold: 0.5 });
